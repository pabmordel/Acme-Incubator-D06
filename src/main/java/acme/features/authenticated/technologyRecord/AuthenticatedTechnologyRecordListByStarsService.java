...
@Service
public class AuthenticatedTechnologyRecordListByStarsService implements AbstractListService<Authenticated, TechnologyRecord> {
	...
	public Collection<TechnologyRecord> findMany(final Request<TechnologyRecord> request) {
		...
		techlist = this.repository.findManyByStars();
		return techlist;
	}
